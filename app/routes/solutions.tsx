          ...
          <div className="grid md:grid-cols-2 gap-12">
            {useCases.map((useCase, index) => (
              <div key={index} className="bg-gradient-to-br from-gray-50 to-valencia-50 rounded-2xl p-8">
                <div className="w-16 h-16 bg-valencia-500 rounded-2xl flex items-center justify-center mb-6">
                ...
                <ul className="space-y-3">
                  {useCase.features.map((feature, featureIndex) => (
                    <li key={featureIndex} className="flex items-center space-x-3">
                      <CheckCircle className="h-5 w-5 text-valencia-500 flex-shrink-0" />
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="grid lg:grid-cols-3 gap-8">
            {options.map((option, index) => (
              <div key={index} className={`bg-white rounded-2xl shadow-lg hover:shadow-xl transition-all duration-300 ${option.popular ? 'ring-2 ring-valencia-500 transform scale-105' : ''}`}>
                {option.popular && (
                  <div className="bg-valencia-500 text-white text-center py-2 px-4 rounded-t-2xl font-medium">
                    Most Popular
                  </div>
                )}
                ...
                <div className="p-8">
                  ...
                  <ul className="space-y-3 mb-8">
                    {option.benefits.map((benefit, benefitIndex) => (
                      <li key={benefitIndex} className="flex items-start space-x-3">
                        <CheckCircle className="h-5 w-5 text-valencia-500 mt-0.5 flex-shrink-0" />
                  ...
                  <Link
                    to="/onboarding"
                    className={`w-full py-3 px-6 rounded-full font-semibold transition-all duration-300 flex items-center justify-center group ${
                      option.popular
                        ? 'bg-valencia-500 text-white hover:bg-valencia-600'
                        : 'border-2 border-valencia-500 text-valencia-500 hover:bg-valencia-500 hover:text-white'
                    }`}
                  >
                    Get Started
